...
class DataProvider extends ChangeNotifier {
  ...
  void filterProducts(String keyword) {
    if (keyword.isEmpty) {
      ...
      _filteredProducts = _allProducts.where((product) {
        ...
        // Can add more conditions here if there are more fields to mach against
  ...
  void filterVariants(String keyword) {
    if (keyword.isEmpty) {
      ...
      final lowerKeyword = keyword.toLowerCase();
      _filteredVariants = _allVariants.where((variantType) {
        return (variantType.name ?? '').toLowerCase().contains(lowerKeyword);
      }).toList();
    }
    notifyListeners();
  ...
  Future<List<MyNotification>> getAllNotifications({bool showSnack = false}) async {
    try {
      Response response = await service.getItems(endpointUrl: 'all-notification');
      if (response.isOk) {
  ...
  int calculateOrdersWithStatus({String? status}) {
    int totalOrders = 0;
    // if status is null return total number of orders
    if (status == null) {
      totalOrders = _allOrders.length;
    } else {
      // for through all orders
      for (Order order in _allOrders) {
        // check if order status matches the target status
        if (order.orderStatus == status) {
          totalOrders += 1; //increments the count if status matches the target
        }
      }
    }
    return totalOrders;
  ...
  void filterProductsByQuantity(String productQtnType) {
    if (productQtnType == 'All Product') {
      _filteredProducts = List.from(_allProducts);
    } else if (productQtnType == 'Out of Stock') {
      _filteredProducts = _allProducts.where((product) {
        // filter products with quantity to 0 (out of stock)
        return product.quantity != null && product.quantity == 0;
      }).toList();
    } else if (productQtnType == 'Limited Stock') {
      _filteredProducts = _allProducts.where((product) {
        //Filter products with quantity equal to 1 (limited stock)
        return product.quantity != null && product.quantity == 1;
      }).toList();
    } else if (productQtnType == 'Other Stock') {
      _filteredProducts = _allProducts.where((product) {
